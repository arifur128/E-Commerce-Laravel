...
<head>
    ...
    <title>All Products - Jersey Shop</title>
            ...
            <div class="flex flex-col md:flex-row justify-between items-center text-xs md:text-sm space-y-2 md:space-y-0">
                <div class="flex items-center space-x-4 md:space-x-6">
                    <span class="flex items-center space-x-2">
                        ...
                        <span class="hidden sm:inline">+880 1234-567890</span>
                    </span>
                    <span class="flex items-center space-x-2">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
                        </svg>
                        <span class="hidden sm:inline">support@jerseyshop.com</span>
                    </span>
                </div>
                <div class="flex items-center space-x-4">
                    <span class="text-[10px] md:text-sm font-medium">🔥 Free Shipping Over TK2000!</span>
        ...
        <div class="container mx-auto px-4 md:px-6">
            <div class="flex justify-between items-center h-16 md:h-20">
                <a href="/" class="flex items-center space-x-2 md:space-x-3 group">
                    <div class="bg-gradient-to-br from-blue-600 to-blue-800 text-white px-3 py-2 md:px-4 md:py-3 rounded-lg md:rounded-xl font-bold text-lg md:text-2xl shadow-lg transform group-hover:scale-105 transition-all">
                        JS
                    </div>
                    <div>
                        ...
                        <p class="hidden md:block text-xs text-gray-500">Premium Football Jerseys</p>
                    </div>
                </a>
                
                <form action="{{ route('search') }}" method="GET" class="hidden lg:block flex-1 max-w-md xl:max-w-xl mx-8">
                    <div class="relative">
                        <input type="text" name="search" placeholder="Search jerseys..." 
                            class="w-full px-6 py-2.5 pl-12 border-2 border-gray-100 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500 transition">
                ...
                <div class="flex items-center space-x-3 md:space-x-6">
                    <div class="hidden md:flex items-center space-x-6">
                        ...
                        <a href="{{ route('all.products') }}" class="text-gray-700 hover:text-blue-600 font-semibold transition">Shop</a>
                    </div>

                    @auth
                        <a href="{{ route('cart.view') }}" class="relative text-gray-700 hover:text-blue-600 p-2">
                            ...
                            @if($cartCount > 0)
                                <span class="absolute top-0 right-0 bg-red-500 text-white text-[10px] font-bold rounded-full h-4 w-4 md:h-5 md:w-5 flex items-center justify-center">
                                    {{ $cartCount }}
                                </span>
                            @endif
                        </a>
                        <a href="{{ route('dashboard') }}" class="hidden sm:flex bg-blue-600 text-white px-4 py-2 md:px-6 md:py-2.5 rounded-full hover:bg-blue-700 transition font-semibold text-sm">
                            Dashboard
                        </a>
                    @else
                        <a href="{{ route('login') }}" class="text-gray-700 hover:text-blue-600 font-semibold text-sm">Login</a>
                        <a href="{{ route('register') }}" class="bg-blue-600 text-white px-4 py-2 rounded-full hover:bg-blue-700 transition font-semibold text-sm">
                            Sign Up
                        </a>
                    @endauth
                </div>
    ...
    <section class="bg-gradient-to-r from-blue-600 to-blue-800 text-white py-16">
        <div class="container mx-auto px-6 text-center">
            ...
            <p class="text-xl text-blue-100">Browse our complete collection of premium football jerseys</p>
        </div>
    </section>
   ...
   <section class="py-10 md:py-16 bg-gray-50">
    <div class="container mx-auto px-4 md:px-6">
        ...
        @if($products->count() > 0)
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8">
                ...
                <a href="{{ route('product.show', $product->slug) }}" class="group">
                    <div class="bg-white rounded-xl md:rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden h-full border border-gray-100">
                        <div class="relative aspect-[3/4] overflow-hidden bg-gray-100">
                            <img src="{{ asset('images/products/'.$product->image) }}" 
                                 alt="{{ $product->name }}" 
                                 class="w-full h-full object-cover transform group-hover:scale-110 transition duration-500"
                                 loading="lazy"> @if($product->discount_price)
                                <span class="absolute top-2 right-2 md:top-4 md:right-4 bg-red-500 text-white px-2 py-0.5 md:px-3 md:py-1 rounded-full text-[10px] md:text-sm font-bold shadow-lg">
                                    SALE
                                </span>
                            @endif
                            
                            <span class="absolute top-2 left-2 md:top-4 md:left-4 bg-blue-600/90 backdrop-blur-sm text-white px-2 py-0.5 md:px-3 md:py-1 rounded-full text-[9px] md:text-xs font-semibold uppercase tracking-wider">
                                {{ $product->category->name }}
                            </span>
                        </div>

                        <div class="p-3 md:p-6">
                            <h3 class="font-bold text-sm md:text-lg mb-2 text-gray-800 group-hover:text-blue-600 transition line-clamp-2 min-h-[40px]">
                                {{ $product->name }}
                            </h3>
                            ...
                            <div class="flex flex-col md:flex-row md:items-center justify-between gap-2">
                                <div>
                                    @if($product->discount_price)
                                        <span class="text-gray-400 line-through text-[10px] md:text-sm block">৳{{ number_format($product->price, 0) }}</span>
                                        <span class="text-blue-600 font-bold text-base md:text-2xl">৳{{ number_format($product->discount_price, 0) }}</span>
                                    @else
                                        <span class="text-blue-600 font-bold text-base md:text-2xl">৳{{ number_format($product->price, 0) }}</span>
                                    @endif
                                </div>
                                <div class="hidden md:block bg-blue-600 text-white p-2 rounded-full hover:bg-blue-700 transition">
        ...
        @else
            <div class="bg-white rounded-2xl shadow-lg p-10 md:p-16 text-center border border-gray-100">
                <div class="w-20 h-20 md:w-24 md:h-24 mx-auto bg-gray-50 rounded-full flex items-center justify-center mb-6">
                ...
                <h3 class="text-xl md:text-2xl font-bold text-gray-800 mb-2">No Products Found</h3>
                <p class="text-gray-500 mb-8 max-w-sm mx-auto text-sm md:text-base">We couldn't find any products in our collection right now. Please check back later.</p>
                <a href="/" class="inline-block bg-blue-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-blue-700 transition shadow-md hover:shadow-lg active:scale-95 transition-all">
                    Return to Homepage
                </a>
            </div>
        @endif
