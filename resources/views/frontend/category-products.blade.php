...
<head>
    ...
    <title>{{ $category->name }} - Jersey Shop</title>
    ...
    <div class="container mx-auto px-6">
        <div class="flex justify-between items-center text-sm">
            ...
            <div class="flex items-center space-x-4">
                <span>🔥 Free Shipping on Orders Over TK2000!</span>
    ...
    <section class="bg-gradient-to-r from-blue-600 to-blue-800 text-white py-16">
        <div class="container mx-auto px-6 text-center">
            ...
            @if($category->description)
                <p class="text-xl text-blue-100 mb-6">{{ $category->description }}</p>
            @endif
            <p class="text-lg">{{ $products->count() }} Products Available</p>
        </div>
    </section>
        ...
        <div class="container mx-auto px-6">
            @if($products->count() > 0)
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                    ...
                    <a href="{{ route('product.show', $product->slug) }}" class="block">
                        <div class="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-all hover-scale">
                            <div class="relative overflow-hidden">
                                <img src="{{ asset('images/products/'.$product->image) }}" alt="{{ $product->name }}" class="w-full h-72 object-cover">
                            ...
                            <div class="p-6">
                                <h3 class="font-bold text-lg mb-3 text-gray-800 hover:text-blue-600 transition">{{ $product->name }}</h3>
                                <div class="flex items-center justify-between">
                                    <div>
                                        @if($product->discount_price)
                                            <span class="text-gray-400 line-through text-sm block">TK{{ number_format($product->price, 0) }}</span>
                                            <span class="text-blue-600 font-bold text-2xl">TK{{ number_format($product->discount_price, 0) }}</span>
                                        @else
                                            <span class="text-blue-600 font-bold text-2xl">TK{{ number_format($product->price, 0) }}</span>
                                        @endif
                                    </div>
                                    <div class="bg-blue-600 text-white p-2 rounded-full hover:bg-blue-700 transition">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @else
                <div class="bg-white rounded-2xl shadow-lg p-16 text-center">
                    ...
                    <p class="text-gray-600 mb-8">This category doesn't have any products yet.</p>
                    <a href="/" class="inline-block bg-blue-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-blue-700 transition shadow-lg">
                        Back to Home
                    </a>
                </div>
            @endif
