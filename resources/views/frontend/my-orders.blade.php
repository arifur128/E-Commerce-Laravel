...
<head>
    ...
    <title>My Orders - Jersey Shop</title>
...
<a href="{{ route('all.products') }}"
   class="flex items-center space-x-2 text-gray-700 hover:text-blue-600 font-semibold transition">
    ...
    <section class="py-16">
        <div class="container mx-auto px-6">
            <h2 class="text-4xl font-bold mb-8 text-gray-800">My Orders</h2>
                    ...
                    <div class="bg-white rounded-xl shadow-md overflow-hidden">
                        ...
                        <div class="p-6">
                            <div class="space-y-3 mb-4">
                                ...
                                <div class="flex items-center space-x-4">
                                    <img src="{{ asset('images/products/'.$item->product->image) }}" class="w-16 h-16 object-cover rounded">
                                    <div class="flex-1">
                                       <p class="font-semibold">{{ $item->product->name }}</p>
@if($item->size)
    <p class="text-sm text-blue-600 font-semibold">Size: {{ $item->size }}</p>
@endif
<p class="text-sm text-gray-500">Qty: {{ $item->quantity }} × TK{{ number_format($item->price, 0) }}</p>
                                    </div>
                            ...
                            <div class="border-t pt-4 flex justify-between items-center">
                                <div>
                                    ...
                                    <p class="font-semibold">{{ ucfirst($order->payment_method) }} - {{ $order->transaction_id }}</p>
                                </div>

                            @if($order->note)
<div class="mt-4 bg-yellow-50 border-l-4 border-yellow-400 p-3 rounded">
    <p class="text-sm text-gray-600 font-semibold mb-1">Order Note:</p>
...
@endif
                                ...
                                <div class="text-right">
                                    ...
                                    <p class="text-2xl font-bold text-blue-600">TK{{ number_format($order->total_amount, 0) }}</p>
                                </div>

                                <div>
    <a href="{{ route('order.invoice', $order->id) }}" class="inline-block bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition text-sm font-semibold">
        Download Invoice
    </a>
</div>
                            </div>
