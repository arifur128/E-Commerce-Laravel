...
<head>
    ...
    <div class="bg-gradient-to-r from-blue-600 to-blue-800 text-white py-2">
        <div class="container mx-auto px-4 md:px-6">
            <div class="flex flex-col md:flex-row justify-between items-center text-[10px] md:text-sm space-y-2 md:space-y-0 text-center">
                <div class="flex items-center space-x-4 md:space-x-6">
                    <span>📞 +880 1234-567890</span>
                    <span class="hidden sm:inline">✉️ support@jerseyshop.com</span>
                </div>
                <span class="font-medium">🔥 Free Shipping on Orders Over TK2000!</span>
            </div>
        </div>
    </div>
            ...
            <div class="flex justify-between items-center h-16 md:h-20">
                <a href="/" class="flex items-center space-x-2 md:space-x-3 group shrink-0">
                    ...
                    <div class="leading-tight">
                        ...
                        <p class="hidden sm:block text-[10px] text-gray-500 font-medium">Premium Football Jerseys</p>
                    </div>
                </a>
                ...
                <div class="flex items-center space-x-3 md:space-x-6">
                    <a href="/" class="text-gray-700 hover:text-blue-600 font-semibold text-sm md:text-base hidden xs:block">Home</a>
                    
                    <a href="{{ route('cart.view') }}" class="relative text-blue-600 font-bold flex items-center p-2">
    ...
    <div class="bg-white border-b overflow-x-auto whitespace-nowrap">
    ...
    <section class="py-6 md:py-16">
        <div class="container mx-auto px-4 md:px-6">
            <h2 class="text-2xl md:text-4xl font-bold mb-6 md:mb-8 text-gray-800">Shopping Cart</h2>

            @if(session('success'))
                <div class="bg-green-100 border-l-4 border-green-500 text-green-700 p-4 rounded mb-6 text-sm">
                ...
                <div class="grid lg:grid-cols-3 gap-6 md:gap-8">
                    <div class="lg:col-span-2 space-y-4">
                        ...
                        @foreach($cartItems as $item)
                            @php 
                                $basePrice = $item->product->discount_price ?? $item->product->price;
                                $priceAdjustment = 0;
                                if($item->size) {
                                    $sizeVariant = $item->product->sizes()->where('size', $item->size)->first();
                                    if($sizeVariant) { $priceAdjustment = $sizeVariant->price_adjustment; }
                                }
                            ...
                            <div class="bg-white rounded-xl shadow-md p-4 md:p-6 hover:shadow-lg transition">
                                <div class="flex flex-row items-start space-x-4 md:space-x-6">
                                    <img src="{{ asset('images/products/'.$item->product->image) }}" class="w-20 h-20 md:w-24 md:h-24 object-cover rounded-lg shrink-0">
                                    
                                    <div class="flex-1 flex flex-col md:flex-row justify-between">
                                        <div class="mb-4 md:mb-0">
                                            <h3 class="font-bold text-base md:text-lg text-gray-800 line-clamp-1">{{ $item->product->name }}</h3>
                                            <p class="text-xs md:text-sm text-gray-500">{{ $item->product->category->name }}</p>
                                            @if($item->size)
                                                <p class="text-xs md:text-sm text-blue-600 font-semibold mt-1">Size: {{ $item->size }}</p>
                                            @endif
                                            <p class="text-blue-600 font-bold text-lg md:text-xl mt-1 md:mt-2">TK{{ number_format($price, 0) }}</p>
                                        </div>

                                        <div class="flex flex-row md:flex-col justify-between items-end md:space-y-3">
                                            <form action="{{ route('cart.update', $item->id) }}" method="POST" class="flex items-center space-x-1 md:space-x-2">
                                                @csrf
                                                <input type="number" name="quantity" value="{{ $item->quantity }}" min="1" class="w-12 md:w-16 px-2 py-1.5 md:py-2 border border-gray-300 rounded-lg text-center text-sm focus:ring-2 focus:ring-blue-500">
                                                <button type="submit" class="bg-blue-600 text-white p-2 md:px-4 md:py-2 rounded-lg text-xs font-semibold hover:bg-blue-700 transition">
                                                    <span class="hidden md:inline">Update</span>
                                                    <svg class="w-4 h-4 md:hidden" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
                                                </button>
                                            </form>
                                            ...
                                            <div class="flex flex-col items-end">
                                                <form action="{{ route('cart.remove', $item->id) }}" method="POST">
                                                    @csrf @method('DELETE')
                                                    <button type="submit" class="text-red-600 hover:text-red-800 font-semibold text-xs flex items-center space-x-1">
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                                        <span class="hidden xs:inline">Remove</span>
                                                    </button>
                                                </form>
                                                <p class="text-gray-800 font-bold text-base md:text-lg mt-1">TK{{ number_format($subtotal, 0) }}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="lg:col-span-1">
                        <div class="bg-white rounded-xl shadow-lg p-5 md:p-6 sticky top-24">
                            <h3 class="text-xl md:text-2xl font-bold mb-4 md:mb-6 text-gray-800 border-b pb-2">Order Summary</h3>
                            
                            <div class="space-y-4 mb-6">
                                <div class="flex justify-between text-sm md:text-base text-gray-600">
                                    <span>Subtotal ({{ $cartItems->count() }} items)</span>
                                    <span class="font-semibold">TK{{ number_format($total, 0) }}</span>
                                </div>
                                ...
                                <div class="border-t pt-4">
                                    <div class="flex justify-between items-center">
                                        ...
                                        <span class="text-2xl md:text-3xl font-bold text-blue-600">TK{{ number_format($total + $shippingCharge, 0) }}</span>
                                    </div>
                                </div>
                            </div>

                            <a href="{{ route('checkout') }}" class="block w-full bg-blue-600 text-white py-3 md:py-4 rounded-xl font-bold text-base md:text-lg hover:bg-blue-700 transition shadow-lg mb-3 text-center">
                                Proceed to Checkout
                            </a>
                            <a href="/" class="block text-center text-sm text-gray-600 hover:text-blue-600 font-medium py-2">
                                ← Continue Shopping
                            </a>

                            <div class="mt-4 pt-4 border-t hidden xs:block">
                                <div class="flex items-center space-x-2 text-xs md:text-sm text-gray-600 mb-2">
            ...
            @else
                <div class="bg-white rounded-2xl shadow-lg p-10 md:p-16 text-center">
                    <svg class="w-16 h-16 md:w-24 md:h-24 mx-auto text-gray-300 mb-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
                    <h3 class="text-xl md:text-2xl font-bold text-gray-800 mb-2">Your Cart is Empty</h3>
                    <p class="text-sm md:text-base text-gray-600 mb-8">Looks like you haven't added any items yet.</p>
                    <a href="/" class="inline-block bg-blue-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-blue-700 transition shadow-lg">Start Shopping</a>
                </div>
            @endif
        </div>
    </section>  
